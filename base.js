...
// Variáveis Globais
var map, baseMaps, bounds_group;
var idsSelecionados = {}, idsSelecionadosComp = {};
var totalComprimento = 0, totalPostes = 0, totalIds = 0;
var highlightLayer;

// Variáveis das camadas - definidas globalmente, mas serão inicializadas dentro de inicializarMapa
var layer_Cabos_2, layer_Postes_3, layer_ProjetosparaCompartilhar_1;

// Crie a instância de Autolinker de forma global
var autolinker = new Autolinker({
    truncate: { length: 30, location: 'smart' },
    newWindow: true, // Abre links em uma nova janela
    className: "my-autolinker" // Classe CSS personalizada para estilização
});
...
function inicializarMapa() {
    ...
    // Códigos de inicialização omitidos para brevidade...

    // Criação e Adição de Camadas Específicas
    layer_Cabos_2 = adicionarCamada('Cabos', 'pane_Cabos_2', 402, 'normal', json_Cabos_2, pop_Cabos_2, style_Cabos_2_0);
    layer_Postes_3 = adicionarCamada('Postes', 'pane_Postes_3', 403, 'normal', json_Postes_3, pop_Postes_3, style_Postes_3_0, true);
...
// Cria e retorna a camada de satélite
...
// Função para criar painéis no mapa
...
function configurarGeocoder() {
    var osmGeocoder = new L.Control.Geocoder({
        collapsed: false, position: 'topright', text: 'Procure', title: 'Testing'
    }).addTo(map);

    var geocoderIcon = document.getElementsByClassName('leaflet-control-geocoder-icon')[0];
    geocoderIcon.className += ' fa fa-search';
    geocoderIcon.title += 'Procure por um local';
}

// Configura eventos para checkboxes
function configurarCheckboxEventos() {
    configurarCheckbox('caixaCabos', layer_Cabos_2);
    configurarCheckbox('caixaPostes', layer_Postes_3);
    configurarCheckbox('caixaCabosCompart', layer_ProjetosparaCompartilhar_1);
}

// Configura um único checkbox para adicionar/remover uma camada
function configurarCheckbox(idCheckbox, camada) {
...
// Chama a função de inicialização após definir todas as funções necessárias
